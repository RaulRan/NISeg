...
load ./Evaluation/Algorithm_Bseg/Ucm2/airplane/image063.mat
...
for i=[63 83]
%     figure
    load(sprintf('%s%03d.mat','./Evaluation/Algorithm_Bseg/Ucm2/airplane/image',i));
    level=1; %(in this case only 1 hierarchical level is available)
    airplane_output_at_level_frame_1= Doublebackconv(segs{level});
    if i == 63
    airplane1 = airplane_output_at_level_frame_1;
    elseif i == 83
    airplane2 = airplane_output_at_level_frame_1;
    end
    Init_figure_no(i), imagesc(airplane_output_at_level_frame_1);
...
% Version History.
%
% Version 1.3 Introduction of parallelization in the computation of BPR,
% VPR and the other metrics. Fix of bug regarding continuation from
% previous partial results.
%
% Version 1.2 Change of video naming and subfolders to fit the original
% BVDS video sequences. The new code is compatible with both the previous and
% current name notations and video structure, so using the (previously)
% downloaded labels with the new code is also possible.
%
